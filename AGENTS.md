...
This module is consumed by **both** `leakix/web` and `l9scan`. Schema
changes (adding/removing/renaming fields in `L9Event`, changing the plugin
interface) require coordination with both consumers. Bump the version and
update both repos' `go.mod` in separate PRs.
...
`L9Plugin` is the interface that scanners implement. It has:
- `Name() string` - Plugin identifier
- `Init() error` - Optional initialization
- `Scan() ([]L9Event, error)` - Produce events
